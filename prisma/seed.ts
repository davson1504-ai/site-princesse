...
async function main() {
  for (const service of services) await prisma.service.upsert({ where: { slug: service.id }, update: {}, create: { name: service.name, slug: service.id, description: service.description, duration: service.duration, price: service.price } });
